...
lay_no_comments(Node, Ctxt) ->
  ...
  tuple ->
      Es = seq(erl_syntax:tuple_elements(Node),
        lay_text_float(","), reset_prec(Ctxt),
        fun lay/2),
  ...
  match_expr ->
      ...
      D2 = lay(erl_syntax:match_expr_body(Node),
          set_prec(Ctxt, PrecR)),
      D3 = follow(beside(D1, lay_text_float(" =")), D2,
  ...
  fun_expr ->
      Ctxt1 = reset_prec(Ctxt),
      Clauses = lay_clauses(erl_syntax:fun_expr_clauses(Node),
          fun_expr, Ctxt1),
  ...
  named_fun_expr ->
      ...
      Clauses = lay_clauses(erl_syntax:named_fun_expr_clauses(Node),
                      {function, D1}, Ctxt1),
      lay_fun_sep(Clauses, Ctxt1);
  ...
  attribute ->
      ...
      lay(N, Ctxt1);
                    _ ->
                        D1 = par(seq(Args, lay_text_float(","), Ctxt1,
                                      fun lay/2)),
      beside(lay(N, Ctxt1),
              beside(text("("),
              beside(D1, lay_text_float(")"))))
  ...
  binary ->
      ...
      Es = seq(erl_syntax:binary_fields(Node),
          lay_text_float(","), Ctxt1, fun lay/2),
      beside(lay_text_float("<<"),
        beside(par(Es), lay_text_float(">>")));
  ...
  record_access ->
      ...
      D3 = beside(beside(lay_text_float("#"),
  ...
  record_field ->
    ...
    none ->
        D1;
    V ->
        par([D1, lay_text_float("="), lay(V, Ctxt1)],
      Ctxt1#ctxt.break_indent)
      end;
  ...
  try_expr ->
      ...
      Es1 = case erl_syntax:try_expr_after(Node) of
          [] -> Es0;
          As ->
        D2 = sep(seq(As, lay_text_float(","), Ctxt1,
                fun lay/2)),
        [text("after"),
          nest(Ctxt1#ctxt.sub_indent, D2)
          | Es0]
      ...
      lay_error_info(E, reset_prec(Ctxt)));
        ...
        function_type ->
            ...
            beside(lay_text_float(Before),
                   beside(D1,
                          beside(lay_text_float(" -> "),
                                 beside(D2, lay_text_float(After)))));
  ...
  record_type_field ->
        ...
        type_union ->
            ...
            Es = par(seq(erl_syntax:type_union_types(Node),
                         lay_text_float(" |"), set_prec(Ctxt, PrecR),
                         fun lay/2)),
...
lay_follow_beside_text_float(D1, D2, Ctxt) ->
    follow(beside(D1, lay_text_float(" ::")),
        D2, Ctxt#ctxt.break_indent).

lay_fun_sep(Clauses, Ctxt) ->
    sep([
        follow(text("fun"), Clauses, Ctxt#ctxt.sub_indent),
        text("end")
    ]).
...
split_string_third([X | _Xs] = S, N, L, As) when
  X >= $0, X =< $9 ->
    split_string_next(S, N, L, As).
...
lay_clauses(Cs, Type, Ctxt) ->
    vertical(seq(Cs, lay_text_float(";"),
