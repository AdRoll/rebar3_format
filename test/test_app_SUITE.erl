...
test_app(_Config) ->
    ...
    State2 = rebar_state:set(State1, format,
                             [{files, ["src/*.erl", "include/*.hrl"]}]),
    State3 = rebar_state:command_parsed_args(State2,
                                             {[{output, "formatted"}], something}),
    {ok, State3} = rebar3_format_prv:do(State3),
    case os:cmd("git --no-pager diff --no-index -- after formatted") of
      "" -> ok;
      Diff ->
          ct:pal("Differences:~n~s", [unicode:characters_to_binary(Diff)]), ct:fail(Diff)
    end.
