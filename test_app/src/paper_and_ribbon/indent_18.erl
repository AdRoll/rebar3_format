...
-record(
    record, {fields = should:be(indented_using:break_indent(1)),
                including :: those:that_use_types(with_very_long_names),
...
match_expr() ->
    ThisVeryVeryVeryLongMatchExpression = should:be(indented_using:break_indent(1)).
...
-type a_type_with_a_very_long_name() :: this_type:definition_doesnt_fit_in(a:line()).
-spec a_function_with_a_very_long_name() -> a_type_with_a_very_long_name().
a_function_with_a_very_long_name() ->
    {specs, " and ", types, should:be(indented_using:break_indent(1))}.
...
list_generator() ->
    [generators || _In <- list:comprehensions(should:be(indented_using:break_indent(1))),
                    _Which <- is:something(that:looks(quite:awful({in, my, opinion})))].
...
when_clause(A, B, C) when is_integer(A), B >= A andalso C rem 2 =:= 0 ->
    the:guard(should:be(indented_using:sub_indent(1)));
