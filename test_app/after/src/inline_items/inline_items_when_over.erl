...
long_tuple() ->
    X = {x1, x2, x3, x4, y1, y2, y3, y4, very_very_long_name_1,
         very_very_long_name_2, very_very_long_name_3},
    Y = {x1, x2, x3, x4, y1, y2, y3, y4, very_very_long_name_1,
         very_very_long_name_2, very_very_long_name_3},
    {X,
     Y,
     {x1, x2, x3, x4, y1, y2, y3, y4, very_very_long_name_1,
      very_very_long_name_2, very_very_long_name_3}}.
...
-spec long_fun() ->
                  fun((X1, X2, X3, X4, Y1, Y2, Y3, Y4, VeryVeryLongName1,
                       VeryVeryLongName2, VeryVeryLongName3) ->
                          {X1, X2, X3, X4, Y1, Y2, Y3, Y4, VeryVeryLongName1,
                           VeryVeryLongName2, VeryVeryLongName3}).
long_fun() ->
    fun(X1, X2, X3, X4, Y1, Y2, Y3, Y4, VeryVeryLongName1, VeryVeryLongName2,
        VeryVeryLongName3) ->
       {X1, X2, X3, X4, Y1, Y2, Y3, Y4, VeryVeryLongName1, VeryVeryLongName2,
        VeryVeryLongName3}
    end.

-spec long_bin() -> binary().
long_bin() ->
    X = <<1, 1, 1, 1, 2, 2, 2, 2, 333333333333333333, 333333333333333333,
          333333333333333333, 333333333333333333>>,
    Y = <<1:1, 1:1, 1:1, 1:1, 4:4, 2:2/little-integer-unit:32,
          2:2/little-integer-unit:32, 2:2/little-integer-unit:32, 3/float,
          3/float, 3/float, 3/float>>,
    <<X/binary, Y/binary, X/binary, Y/binary, X/binary, Y/binary, X/binary,
      Y/binary, X/binary, Y/binary>>.
...
long_lc() ->
    [{X, Y, Z}
     || X <- generator:x(), X < 1, Y <- generator:y(), Z <- generator:z(),
        filter:x(Y, Z), X + Y == Z].
...
long_bc() ->
    << <<X, Y, Z>>
       || <<X>> <- generator:x(), X < 1,
          <<Y:2/signed-integer-unit:8>> <- generator:y(), Z <- generator:z(),
          filter:x(Y, Z), X + Y == Z >>.

-spec long_arglist(number(), number(), number(), float(), non_neg_integer(),
                   non_neg_integer(), non_neg_integer()) ->
                      number().
long_arglist(X1, X2, X3, Y1, VeryVeryLongName1, VeryVeryLongName2,
             VeryVeryLongName3) ->
    X1
    + X2
    + X3
...
short() ->
    [these, items, should, 'not', be, inlined, since, they, are, less, than,
     25].

exact() ->
    [these, items, should, 'not', be, inlined, $(, i, ".", e, ". ", each, item,
     should, occupy, a, signle, line, $), since, they, are, exactly, 25, $.].
